...
const ManageInhabitantsContent = () => {
        ...
        try {
            ...
            const response = await fetch("http://localhost:8080/api/admin/tenants", {
                headers: {
                    ...
                    "Content-Type": "application/json",  // Jeśli wymagane
                },
            });
            if (!response.ok) {
            ...
            setResidents(data.content); // Ustawiamy mieszkańców z pola "content"
        } catch (err: any) {
    ...
    useEffect(() => {
        fetchResidents(); // Wywołanie pobierania danych przy załadowaniu komponentu
    }, []);
